...
> 审计日期：2026-08-20
>
> Pinned Baseline：[`feat/local-qa-runtime@4b17389711fc420bfef56765d7d6af34e1702eb0`](https://github.com/ChronoAIProject/fkst-hosted/commit/4b17389711fc420bfef56765d7d6af34e1702eb0)
>
> Live status overlay（2026-08-21）：`feat/local-qa-runtime@e91154e89eda84fef2ff7d86623b586be60fd792`；`develop@5af95163cbcdad5dcffac1cc17418bc5417ba98c`。以下历史缺陷只对 pinned Baseline 成立；在 live head 重新核实前，不得继续描述为当前 blocker。`#6009` Journal v4 reopen issue 已 closed/completed。本文仍将 production assembly、Talos adapter 和 AI/tool loop 视为 Target/Candidate，不外推为 `develop` 已交付能力。
>
...
>
> Talos Tool/QARun/attempt/fence 缺口见 [talos 详细缺口](talos-gap-analysis.zh-CN.md)；PQL client/运行投影见 [product-quality-loop 详细缺口](product-quality-loop-gap-analysis.zh-CN.md)；Hosted Authorization/Artifact 仍以 decision-pending consumer contract 为准；Final Quality/Report 属于后续外部领域。
...
- 执行中 cancel、executor error、restart stranded attempt、通用 AI/tool loop 和 Talos adapter 仍是 Target/Candidate，必须以 live head 和 conformance gate 重新验证。
...
**P0：** 重新审计 live head；定义通用 Runtime Core、Executor identity、ModelObservation、typed tool broker 和 AI-session-facing admission；接通真实执行、取消、失败恢复和 canonical result commit。Pinned Baseline 的 Journal v4 reopen 只作为历史验证项，不能直接作为 live blocker。
...
**明确不归 Runtime：** Talos QARun/placement/lease/fence 与生产 `LocalQARuntimeAdapter`、PQL selection、Testing Packages 的 AI TestCaseExecutionEngine/AssertionReducer、Hosted Authorization/Artifact owner 和 Quality/Report/Settlement。
...
Pinned Baseline 的这些能力没有被 production Host 串联，并存在 Journal v4 reopen bug。该缺陷已由 #6009 标记 closed/completed，但 live `e91154e8...` 必须通过代码和 reopen tests 重新验证；本文不再把它作为未经验证的当前第一 blocker。
...
machine / worker / installation identity
...
所有验证必须发生在 workspace、process、port、model call、tool effect、Chrome 或 Evidence staging effect 之前。Runtime 必须验证 local credential、operation-specific authorization、attempt/fence currentness、executor identity、tool catalog、model/prompt/harness identity 和 durable budget；任一验证不可用时 fail closed。Runtime 不保存 NyxID bearer、Talos worker token、provider secret 或 raw prompt/observation，只消费已投影且可本地验证的 bounded context。
...
| --- | --- | --- | --- | --- |
| Contract convergence | scalar/ref/protocol 基础 | 通用 Run request、TestCase input、Executor identity、AI session/tool/result contracts、bounds/outcomes/errors/receipts | `qa-contracts` canonical/digest validators、MVP fixtures | Rust/TS 对合法、冲突、工具拒绝、预算耗尽和 failpoints 给出同一结果 |
...
| Admission | inert body + fixed digest | case-set、tool catalog、executor/model identity、policy、budgets、attempt/fence、idempotency、deadline 原子提交 | 当前 WAL Journal/admit transaction | 任一 identity/capability/authorization mismatch 零 effect |
...
- machine-readable payload/string/array/depth/Event/Observation/Evidence/staging/TTL bounds。
- shared Rust/TypeScript fixtures、live-baseline overlay、failpoint expectations。
...
- 真实 admission request 经过 local credential、Talos run/task/attempt、generation/fence、executor/model/tool identity、deadline、digest、idempotency 和 single-active gate。
